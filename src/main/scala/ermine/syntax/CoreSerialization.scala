...
object CoreSerialization {
  ...
  def coreW1: Writer1[Core] = new Writer1[Core] {
  ...
  def coreW[V](vw: Writer[V, DynamicF]): Writer[Core[V], DynamicF] = {
    ...
    )((a,b,c,d,e,f,g,h,i) => (core: Core[V]) => core match {
      case Var(v)            => a(v)
      case Data(t, fs)       => b((t, fs))
      case App(f, x)         => c((f, x))
      case Lam(ar, body)     => d((ar, body))
      case Let(bs, b)        => e((bs, b))
      case Case(c, bs)       => f((c, bs))
      case Dict(sups, slots) => g((sups, slots))
      case LamDict(b)        => h(b)
      case AppDict(f, d)     => i((f, d))
    }).erase
  }
}
